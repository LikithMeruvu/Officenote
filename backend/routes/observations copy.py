...
def perform_qa_v2(pdf_stream, toc_df, questions_data, output_dir, openai_api_key):
    """
    Performs Q&A for questions with multiple subcategories and page ranges.
...
def process_pdf_v2(pdf_file, excel_df, openai_api_key):
    """
    Main function to process the PDF and Excel DataFrame (version 2), perform Q&A, and return results.
    ...
    # Perform Q&A
    qa_results = perform_qa_v2(pdf_stream, toc_df, questions_data, tempfile.gettempdir(), openai_api_key)
...
async def process_file_v2(pdf: UploadFile = File(...), excel_file: UploadFile = File(...)):
    """
    ...
    Handles questions with multiple subcategories and page ranges:
      - If 'range_cX' == '-1', extracts the full sub-PDF.
      - Otherwise, extracts only 'range_cX' pages from the start of that subcategory.
      - If 'range_cX' == '0' or is missing, the subcategory is skipped.
    """
    if pdf.content_type != "application/pdf":
        raise HTTPException(status_code=400, detail="Uploaded file is not a valid PDF.")
    ...
    try:
        qa_results = process_pdf_v2(pdf, excel_df, openai_api_key)
    except HTTPException as he:
        raise he
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"An unexpected error occurred: {e}")
...
app.include_router(router) # Include the router in the main application